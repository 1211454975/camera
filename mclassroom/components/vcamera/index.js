...
Component({
  ...
  methods: {
    ...
    initReady(){
      ...
      console.log("页面准备好了, 读取参数为：",this.properties)
    ...
    render(frame) {
      ...
      if (!anchor2DList || anchor2DList.length <= 0) {
        ...
        //上报父组件这些捕获的肢体位置数据
        this.triggerEvent("motion", anchor2DList)
        if (!initShadersDone) {
    ...
    toggleCamera(direction){
      console.log("toggleCamera==>", direction)
      if(!this.session||!this.session.config){
        return;
      }
      let config = this.session.config
      let cameraPosition = direction=="1"?1:0
      config.cameraPosition = cameraPosition
      this.session.config = config
      console.log("toggleCamera end==>", this.session.config)
    }
  }
})
