...
const MotionBase = require("../../libs/MotionBase");
...
Component({
  ...
  data: {
    ...
    enableTip: false,
    enableRuleTip: false,
    enableBoneFrame: true,
    enableBodyFrame: true,
  ...
  lifetimes: {
    ...
    ready() {
      console.log("页面准备完全:",this.properties.rule)
      var rule = this.properties.rule||{}
      //初始化motionBase
      var motionBase = new MotionBase(rule.key,rule.name,rule.rule);
      motionBase.onTick = (times) =>{
        this.setData({
          times:times
        })
      }
      ...
      setInterval( ()=>{
        var duration = Date.now() - this.data.startTime;
        var s = Math.floor(duration/60000)
        var m = Math.floor((duration-60000*s)/1000)
        var str = ""
        if(s<10) str= "0"+s+":"
        else str = s+":"
        if(m<10) str = str + "0"+m
  ...
  methods: {
    ...
    initCover(){
      ...
      query.select("#canvas")
        ...
        .exec((res) => {
          ...
          //init body-marker
          console.log("canvas:::",canvas.width,canvas.height, pixelRatio)
          var bodyMarker = new Body.marker({canvas:canvas,pixelRatio:pixelRatio})
          bodyMarker.init()
        })
    },
    switchCamera(event){
    ...
    render(frame) {
      ...
      if (!anchor2DList || anchor2DList.length <= 0) {
        ...
        //计数处理，异步逻辑
        this.motionBase.check(anchor2DList)
      }
    },
    ...
    toggleCamera(event){
      if(this.session.config){
        const config = this.session.config
        let pos = Number(this.data.cameraPosition)
        pos = pos==0?1:0;
        config.cameraPosition = pos
        this.session.config = config
        this.setData({
    ...
    openSetting(event){
      console.log("enableOpenSetting:", this.data.enableOpenSetting)
      this.setData({
        enableOpenSetting: !this.data.enableOpenSetting
      })
    },
    //Setting Form 事件处理//////////////////
    enableTipChange(event){
      this.setData({
        enableTip: event.detail.value
      })
    },
    enableRuleTipChange(event){
      this.setData({
        enableRuleTip: event.detail.value
      })
    },
    enableBoneFrameChange(event){
      this.setData({
        enableBoneFrame: event.detail.value
      })
    },
    enableBodyFrameChange(event){
      this.setData({
        enableBodyFrame: event.detail.value
      })
    },
    //显示插件库，并进行选择
    showPluginDialog(event){
      this.triggerEvent("toggle-plugin")
      // this.setData({
      //   showPluginListDialog: !this.data.showPluginListDialog
      // })
      //console.log("showPluginListDialog:", this.data.showPluginListDialog)
    },
