...
function processVideo(stream) {
    ...
    movieMaterial = new THREE.ShaderMaterial({
        uniforms: {
            ...
            uMaxFreq: { type: "1f", value: null },
            uMaxFreq2: { type: "1f", value: null },
			uMaxFreq3: { type: "1f", value: null },
			uMaxFreqLoc: { type: "1i", value: null },
			uMaxFreq2Loc: { type: "1i", value: null },
			uMaxFreq3Loc: { type: "1i", value: null }
...
function processAudio(stream) {
    ...
    recorder.onaudioprocess = function(s) {
        ...
        for (var i = 0; i < (audioSliceSize*numChannels); i++) {
        	var freqNorm = 1.0 - (Math.abs(audioFreqQueue[i])/128.0);
            audioFreqSlice.push(freqNorm);
            if (freqNorm > maxf && i%2 == 0) {
            	maxf3 = maxf2;
            	maxf3Loc = maxf2Loc;
            	maxf2 = maxf;
            	maxf2Loc = maxfLoc;
            	maxf = freqNorm;
            	maxfLoc = i;
            } else if (freqNorm > maxf2 && i%2 == 0) {
            	maxf3 = maxf2;
            	maxf3Loc = maxf2Loc;
            	maxf2 = freqNorm;
            	maxf2Loc = i;
            } else if (freqNorm > maxf3 && i%2 == 0) {
            	maxf3 = freqNorm;
            	maxf3Loc = i;
            }
        }
...
//            console.log(maxf + ' ' + maxf2 + ' ' + maxf3 + ' | ' + maxfLoc + ' ' + maxf2Loc + ' ' + maxf3Loc);
            movieMaterial.uniforms.uMaxFreq.value = maxf;
            movieMaterial.uniforms.uMaxFreq2.value = maxf2;
            movieMaterial.uniforms.uMaxFreq3.value = maxf3;
            movieMaterial.uniforms.uMaxFreqLoc.value = maxfLoc;
            movieMaterial.uniforms.uMaxFreq2Loc.value = maxf2Loc;
            movieMaterial.uniforms.uMaxFreq3Loc.value = maxf3Loc;
